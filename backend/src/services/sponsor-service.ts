...
class SponsorService {
  ...
  private async pickGasCoin(requiredBalance: bigint) {
    const coins = await this.client.getCoins({ owner: this.address, coinType: SUI_TYPE, limit: 100 });
    const suitable = coins.data.find((coin) => BigInt(coin.balance) > requiredBalance);
    if (!suitable) {
  ...
  async prepareSponsoredTransaction(options: {
    ...
    const coin = await this.pickGasCoin(config.sponsorGasBudget * BigInt(referenceGasPrice));

    const tx = Transaction.fromKind(fromBase64(options.transactionBlockKindBytes));
  ...
  async executeSponsoredTransaction(digest: string, claimerSignature: string) {
    ...
    const sponsorSignature = (await this.keypair.signTransaction(cached.txBytes)).signature;
