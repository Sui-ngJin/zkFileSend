...
router.post('/transaction-blocks/sponsor', requireSession, async (req: AuthenticatedRequest, res) => {
  ...
  if (body.network && body.network !== config.enokiEnv && body.network !== config.network) {
    return res.status(400).json({ error: `Unsupported network ${body.network}` });
  }
...
router.post('/transaction-blocks/sponsor/:digest', requireSession, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const execution = await sponsorService.executeSponsoredTransaction(req.params.digest, parse.data.signature);
    res.json({ data: { digest: execution.digest } });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to execute sponsored transaction';
