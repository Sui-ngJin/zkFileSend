...
type SignPersonalMessageFn = (params: { message: Uint8Array }) => Promise<{ signature: string }>;
...
export class SealService {
  ...
  constructor() {
    ...
    this.walrus = new WalrusClient({ network: NETWORK, suiClient: this.suiClient });
  ...
  async encryptAndUploadWithWallet(
    ...
    try {
      ...
      if (!this.seal) {
        throw new Error('Seal client not initialized. Please check server configuration.');
      }
  ...
  async decryptAndDownloadWithWallet(
    ...
    try {
      ...
      try {
        ...
        console.log('First 20 bytes of raw data:', Array.from(encrypted.slice(0, 20)));
        ...
        const decrypted = await this.seal.decrypt({ data: encrypted, sessionKey, txBytes });
