...
export const SUI_RPC = import.meta.env.VITE_SUI_RPC || `https://fullnode.${NETWORK}.sui.io:443`;

// Package and policy IDs from deployed contracts
export const PACKAGE_ID = import.meta.env.VITE_PACKAGE_ID;
export const POLICY_ID = import.meta.env.VITE_POLICY_ID;
export const CONTENT_ID_HEX = import.meta.env.VITE_CONTENT_ID_HEX || '6a6f686e5f646f65';

// Seal server configuration
export const SEAL_SERVER_IDS = (import.meta.env.VITE_SEAL_SERVER_IDS || '').split(',').filter(Boolean);
export const SEAL_SERVER_WEIGHTS = (import.meta.env.VITE_SEAL_SERVER_WEIGHTS || '1,1').split(',').map(Number);

// Walrus storage configuration
export const WALRUS_EPOCHS = Number(import.meta.env.VITE_WALRUS_EPOCHS || '5');
export const WALRUS_DELETABLE = (import.meta.env.VITE_WALRUS_DELETABLE || 'true') === 'true';

// Access control
export const OPEN_AFTER_MS = BigInt(import.meta.env.VITE_OPEN_AFTER_MS || '0');
export const ALLOWLIST = (import.meta.env.VITE_ALLOWLIST || '').split(',').filter(Boolean);
