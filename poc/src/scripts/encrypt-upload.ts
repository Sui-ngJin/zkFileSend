...
import { PACKAGE_ID, CONTENT_ID_HEX, WALRUS_DELETABLE, WALRUS_EPOCHS, SUI_SECRET_KEY } from '../lib/env.js';
...
(async () => {
  ...
  const result = await walrus.writeBlob({
    ...
    signer: signer as unknown as NonNullable<Parameters<typeof walrus.writeBlob>[0]>['signer'],
  });
  ...
  console.log('\nWalrus URL:');
  console.log(`https://aggregator.walrus-testnet.walrus.space/v1/blobs/${result.blobId}`);
