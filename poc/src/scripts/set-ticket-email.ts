...
if (!ticketId) {
	throw new Error(
		"Set TICKET_ID in the environment before setting the ticket email",
	);
}

if (!receiverEmailRaw) {
	throw new Error(
		"Set RECEIVER_EMAIL in the environment to bind the ticket to an email",
	);
}

const receiverEmailBytes = Array.from(
	new TextEncoder().encode(receiverEmailRaw.trim().toLowerCase()),
);
