...
if (!Number.isInteger(mintCount) || mintCount <= 0) {
  throw new Error(`Set TICKET_MINT_COUNT to a positive integer (received "${mintCountRaw}")`);
}
...
(async () => {
  ...
  if (ticketIds.length === 0) {
    console.warn('Mint executed but no ticket IDs returned. Inspect transaction digest:', res.digest);
  }
