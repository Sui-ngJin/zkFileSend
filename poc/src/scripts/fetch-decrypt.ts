...
import { PACKAGE_ID, POLICY_ID, CONTENT_ID_HEX, SUI_SECRET_KEY } from '../lib/env.js';
...
if (!ticketId) {
  throw new Error('Set TICKET_ID in the environment to use fetch-decrypt-ticket');
}
...
if (!claimerEmailRaw) {
  throw new Error('Set CLAIMER_EMAIL in the environment with the email used for this ticket');
}

const claimerEmailBytes = Array.from(new TextEncoder().encode(claimerEmailRaw.trim().toLowerCase()));
...
(async () => {
  ...
  const decrypted = await seal.decrypt({ data: encrypted, sessionKey, txBytes });
