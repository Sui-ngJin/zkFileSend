...
export function signerFromEnvKey(input: string): Ed25519Keypair {
	...
	if (input.startsWith("suiprivkey")) {
		...
		if (secretKey.length !== 32)
			throw new Error(`Wrong secretKey size from bech32: ${secretKey.length}`);
	...
	throw new Error(
		`Wrong secretKey size. Expected 32 or 33 bytes, got ${raw.length}. (Check base64 padding "=" or provide suiprivkey)`,
	);
}
